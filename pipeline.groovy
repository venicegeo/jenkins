...
for(i in pzprojects) {
  pipelineJob("venice/l2/piazza/${i.name}-pipeline") {
    ...
    parameters {
      ...
      stringParam("PHASE_ONE_PCF_SPACE", "pz-int", "Phase one Cloudfoundry space")
      stringParam("PHASE_ONE_PCF_DOMAIN", "int.dev.east.paas.geointservices.io", "Phase one Cloudfoundry domain")
      stringParam("PHASE_TWO_PCF_SPACE", "pz-test", "Phase two Cloudfoundry space")
      stringParam("PHASE_TWO_PCF_DOMAIN", "test.dev.east.paas.geointservices.io", "Phase two Cloudfoundry domain")
      ...
      booleanParam("USE_GEOSERVER_PKI_AUTH", false, "Determines if the GeoServer PKI Auth should be applied to apps or not")
